...
####### extracting word embeddings for both TRAIN and TRAIN-ALL settings
    for dataset_dir in dataset_dirs:
        fname_vocab = os.path.join(dataset_dir, 'vocab.pickle')
        vocabulary = cPickle.load(open(fname_vocab))
        words = vocabulary.keys()

        fname ='../embeddings/glove.6B.50d.txt'
...
####### creating and saving in a file the vocabulary of these word embeddings
        ...
        outfile = os.path.join(dataset_dir, 'emb_{}.npy'.format(os.path.basename(fname)))
